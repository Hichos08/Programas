...
if(isset($_SESSION["Operario_login"]))	//Condicion personal
{
	...
	else if($usuario AND $password AND $role)
	{
		try
		{
			$select_stmt=$db->prepare("SELECT username,password,role FROM mainlogin
										WHERE
										username=:usuario AND password=:upassword AND role=:urole"); 
					...
					if($usuario==$dbusuario and $password==$dbpassword and $role==$dbrole)
					{
						switch($dbrole)		//inicio de sesión de usuario base de roles
						{
							case "admin":
								...
								$loginMsg="Admin: Inicio sesión con éxito Entrando...";	
								header("refresh:3;admin/admin_portada.php");	
								break;
								
							case "Operario";
								$_SESSION["Operario_login"]=$usuario;				
								$loginMsg="Operario: Inicio sesión con éxito Entrando...";		
